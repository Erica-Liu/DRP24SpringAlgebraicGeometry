...
- $f(x, y) = x^2 - y^2$
<p style="text-align: center"><img align="" width="" height="" src="https://hackmd.io/_uploads/r1cTjBIFA.gif">


### Polynomial Ring
A <span style="color:blue">**Polynomial Ring**</span> in $x_1, ..., x_n$ with coefficients over field $k$ is the set of all formal sums (polynomials) in the form: $\sum_{n} \alpha_n  x^{n}, \space \alpha_n \in k,$ denoted by
\begin{align*}
k[x_1, ..., x_n]
\end{align*}
...
### Zero Locus
The <span style="color:blue">**Zero Locus**</span> of a function $f$ is the set of points such that $f = 0$.
...
### Affine Spaces
An n-dimensional <span style="color:blue">**Affine Space**</span> over $k$ is the set
\begin{align*}
...
Given field $k$, and polynomials $f_1, ..., f_s \in k[x_1, ..., x_n]$, the <span style="color:blue">**Affine Variety**</span> defined by polynomials $f_1, \dots, f_s$ is a subset of $k^n$ where:
\begin{align*}
...
#### Examples
Revisiting an earlier example, <span style="color:blue">the affine variety for the unit circle</span>: $V(x^2+y^2-1)$
...
Intuitively, <span style="color:blue">a polynomial ideal describes a set of polynomials with a common set of vanishing points (points where the polynomials evaluate to zero). </span>

We will often represent ideals in two ways: 
- We write <span style="color:blue">$\langle f_1, ..., f_s\rangle$</span> to denote the ideal that is generated by $f_1, ..., f_s$. Essentially, $\langle f_1, ..., f_s\rangle$ is the set of all functions of the form $\sum_{i = 1}^s h_if_i$ where $h_i \in k[x_1, ..., x_n]$.
- We write <span style="color:blue">$I(V)$</span> to denote the set of all the functions that vanish on the points in the variety, $V$.

## The Ideal Membership Problem
Now that we have defined ideals and their properties, let's take a different approach. <span style="color:blue">Consider $f_1, ..., f_s \in k[x_1, ..., x_n]$, how can we know if $f$ lies in ideal $\langle f_1, ..., f_s \rangle$?</span> Based on the definition, we can check through conditions if given an 'easy' ideal, whether polynomial $f$ is a member or not. Intuitively, we want to find polynomials $h_1, h_2, ..., h_s \in k[x_1, ..., x_n]$ such that:
\begin{align*}
f = f_1h_1 + ... + f_sh_s 
\end{align*}
...
x^2y+x = x(xy+1)
\end{align*}
- <span style="color:blue">Is $x^2y-y \in \langle x^2 - xy - 1, xy + 1 \rangle$?</span>
Yes!
\begin{align*}
...
Through the examples, it's very apparent that polynomial <span style="color:blue">$f$ is contained within an Ideal if it is divisible by elements of that Ideal.</span> In other words, the remainder eventually becomes 0 after applying divisions.

A question that arose for us when more variables were involved: 
<span style="color:blue">**Is the final remainder unique?**</span>
...
We introduced <span style="color:blue">**Lexicographical Order**</span>:
...
<span style="color:blue">When working with polynomials we will use $x, y, z...$ and assume that ordering is by alphabetical order unless stated otherwise: $x > y > z$.</span>
...
An <span style="color:blue">**example**</span> of how this algorithm would work is: divide $f = xy^2 + 1$ by $f_1 = xy+1$ and $f_2 = y+1$.
...
To solve the ideal membership problem, we want be able to describe any ideal by some finite generating set, to make it easier to see if a polynomial can be created by this set and is therefore in the ideal. In our readings, we also noticed that <span style="color:blue">some ideals have multiple representations.</span> For example,\begin{align*} \langle x+xy, y+xy,x^2, y^2\rangle = \langle x,y\rangle \end{align*} and \begin{align*} \langle 2x^2+3y^2-11, x^2-y^2-3 \rangle = \langle x^2-4, y^2-1\rangle \end{align*} Evidently, <span style="color:blue">$\langle x,y\rangle$ and $\langle x^2-4, y^2-1\rangle$ seem simpler</span> and nicer to work with, but this raised some questions like:
...
As it turns out, <span style="color:blue">most of these questions are answered by what's called a Gröbner basis,</span> which gives us a finite generating set of polynomials for an ideal. Gröbner bases and polynomial division also lead to a neat solution to the ideal membership problem!.

### Our observation

While working with the idea of how we could simplify ideals, we observed the following:

Given an ideal $I \in K[x_1, x_2, ..., x_n]$, and $f_1, f_2, f_3\in I$, <span style="color:blue">we can reduce the complexity of some ideal generated by $f_1, f_2, f_3$ </span>using the following three steps:
...
First, monomial ideal can be thought of as an ideal generated by monomials. 
We denote a monomial ideal as $\langle x^a : a \in A \rangle$, where $A$ is an indexing set.

Dickson's lemma:
<span style="color:blue">*Let $I = \langle x^a : a \in A \rangle \subseteq k[x_1, ..., x_n]$ be a monomial ideal. Then $I$ can be written in the form $I=\langle x^{a_1}, ..., x^{a_s} \rangle$ with $a_1, ..., a_n \in A$*</span>
...
<span style="color:blue">*Every ideal $I \subset k[x_1, ..., x_n]$ has a finite generating set. That is, $I = \langle g_1, ..., g_t \rangle$ for some $g_1, ..., g_n \in I$*</span>
...
Formally, a <span style="color:blue">**Gröbner Basis** </span>for an ideal is a subset <span style="color:blue">$G=\{g_1, ..., g_t\} \subset I$ where $\langle LT(g_1), ..., LT(g_t)\rangle = \langle LT(I)\rangle$</span> where we let $LT(f)$ be the leading term of the polynomial $f$, and where  $\langle LT(I)\rangle$ is the monomial ideal constructed by the leading terms of each the polynomials in $I$. 

Using elements of the proof of Hilbert Basis theorem, we can show that <span style="color:blue">every ideal $I$ has a Gröbner Basis,</span> and that any <span style="color:blue">Gröbner Basis is a basis for $I$</span>. 
...
With a fixed ordering on the variables $x_1, ..., x_n$ in $k[x_1, ..., x_n]$, for any ideal $I \subseteq k[x_1, ..., x_n]$ we can find the Gröbner Basis for $I$ (which gives us a nice generating set for our ideal). Then take any polynomial $f \in k[x_1, ..., x_n]$ divide it by the elements of the Gröbner Basis for $I$. <span style="color:blue">If the remainder is $0$,</span> $f$ can be made from a combination of the basis polynomials that generate $I$, so <span style="color:blue">$f$ must be in $I$,</span> and otherwise, $f$ is not in $I$, solving the ideal membership problem!
...
A cool application of what we learned is that <span style="color:blue">given a 9x9 Sudoku puzzle, we could solve it by using a Gröbner Basis!</span> The paper, **"Gröbner Basis Representations of Sudoku"**, talks about representing Shidoku puzzles (4x4 Version of Sudoku) in polynomials, in a few different methods which made us curious of if we could expand these methods to 9x9 Sudoku boards. 
...
The roots of unity system is one of the methods for encoding a Sudoku puzzle in polynomials that uses roots of unity. The general idea is that we will <span style="color:blue">map each number from 1 to 9 in the original puzzle to one of the 9th roots of unity</span>, and then will <span style="color:blue">solve the puzzle in terms of roots of unity</span>, and will then <span style="color:blue">map our roots of unity back to the numbers 1 to 9</span> to get the solution to our original numbers.

Recall that a 9th root of unity is of the form <span style="color:blue">$e^{\frac{2x \pi\cdot i}{9}} = \cos (\frac{2\pi x}{9}) + i\cdot \sin(\frac{2\pi x}{9})$</span>, for $x \in \mathbb{Z}$. Representing these roots visually, we have:
<figure>
  <p style="text-align: center"><img src="https://hackmd.io/_uploads/BJVguADYC.png" alt="Roots of unity diagram, around a circle">
</figure> 

The following table illustrates our <span style="color:blue">bijection between the integers 1 to 9 and the 9th roots of unity</span>, and shows how each root will be represented in SageMath, which will become important later:
...
1. Suppose <span style="color:blue">each individual box on the Sudoku board is represented by a variable $x_{ij}$</span> for $1 \leq i,j \leq 9$, where $x_{i,j}$ is represented as the $i, j$ coordinate of the board, which visually looks as follows: <p style="text-align: center"><img src="https://hackmd.io/_uploads/BJV8H0vKR.png" alt="Visual of roots of unity diagram"> <p style="text-align:left">This means that we set up $I$ to be an ideal in a polynomial ring of 81 variables! Since we will be working with roots of unity which are complex, we get <span style="color:blue">$I \subset \mathbb{C}[x_{1,1}, x_{1,2}, ..., x_{1, 9}, ..., x_{9,1}, x_{9, 2}, ..., x_{9,9}]$</span>
...
```
2. We first create a list ```gens``` to add all the equations representing $I$ to, and then we first add the restrictions of <span style="color:blue">$x_{ij}^9-1=0$</span> to this set, by looping through all our variables:
```python
gens = []
...
```
After constructing the set of the equations we want in our ideal, we turned the list ```gens``` into a set, <span style="color:blue">constructed an ideal $I$ on from our set of generators, and computed the Gröbner basis B on $I$,</span> and printed the results:
```python
X= Set(gens)
...
#### The Model
Another system that effectively models the 9x9 Sudoku puzzle is the boolean system where <span style="color:blue">binary variables</span> are used to determine if the square takes on a certain value out of ${1, 2,..., 9}$.
...
<img src="https://hackmd.io/_uploads/S1-gxJAOR.png)" alt="drawing" width="500"/>

Let's define the constraints we need for this model.
1. For each square individual slot $x$, we must have that:
<span style="color:blue">\begin{align*}
\sum_{k=1}^9{x_k} = 1
\end{align*}</span>
2. For all 9 relevant slots in each row, column and 3x3 square for a value k we must have that:
<span style="color:blue">\begin{align*}
\sum_{n=1}^9{x_k} = 1
\end{align*}</span>
3. <span style="color:blue">For every pair of slots $x$ and $w$ in any **row**, **column**, and **3x3 square**</span> of a 9x9 puzzle, we must have that the values they take on do not overlap:
<span style="color:blue">\begin{align*}
\sum_{k=1}^9{x_k}{w_k} = 0
\end{align*}</span>
	<span style="color:blue">Either $x_k = 0$ or $w_k = 0$ for every $k$.</span>

#### The Implementation

1. Set up <span style="color:blue">polynomial ring for our Ideal</span>, accounting for $9*9*9 = 729$ variables. Define and set up our starting puzzle.
```python
import itertools
...
2. Start creating our set of polynomial equations for our Ideal. Given a starting puzzle, add the solution set of equations to be equal to 0 for <span style="color:blue">existing values</span> already in the board. These defining polynomials are added to set of equations.
...
```
5. Given all our defining polynomials: <span style="color:blue">Generate the Ideal and Gröbner basis corresponding to the constraint polynomials.</span> The Gröbner basis distinguished the <span style="color:blue">81 boolean variables</span> that are true. This is to be mapped into the solved Sudoku puzzle.
```python      
...
Out of 729 elements in the Gröbner basis the 81 binary variables corresponding to the solution set were represented in a distinct way. They were easily converted to their respective values and positions in the solved puzzle! This can be done with any starting puzzle with a solution.
